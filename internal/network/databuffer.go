...
func (db *DataBuffer) GetShort() (int16, error) {
	...
	v := int16(binary.BigEndian.Uint16(db.buf[db.pos:]))
	db.pos += 2
	return v, nil
}
...
func (db *DataBuffer) GetInt() (int32, error) {
	...
	v := int32(binary.BigEndian.Uint32(db.buf[db.pos:]))
	db.pos += 4
	return v, nil
}
...
func (db *DataBuffer) GetLong() (int64, error) {
	...
	v := int64(binary.BigEndian.Uint64(db.buf[db.pos:]))
	db.pos += 8
	return v, nil
}
...
func (db *DataBuffer) GetFloat() (float32, error) {
	...
	bits := binary.BigEndian.Uint32(db.buf[db.pos:])
...
func (db *DataBuffer) GetDouble() (float64, error) {
	...
	bits := binary.BigEndian.Uint64(db.buf[db.pos:])
